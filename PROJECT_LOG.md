...
## 2026-09-25 — AdSense "low value content" audit + remediation (code + reversible commands)
- Full audit of the live site (now **~2,267 published articles**, up from 357) + codebase against the third AdSense "low value content" rejection. Findings + the run-book live in **`ADSENSE_AUDIT.md`** (repo root). **No PHP/DB/deploy access in this environment** — article data is in prod Postgres, so data changes ship as reversible artisan commands for the server.
- **Technical is healthy** (verified live): robots 200, sitemap.xml 200 (2,986 URLs), news-sitemap 200, self-referential canonical, `max-image-preview:large` (no noindex), server-rendered crawlable body, byline on 100%, thin content ~0% (400w floor works). So the rejection is **content-quality/scale**, not technical.
- **Confirmed defects:** literal `[[LINK]]` shipped in an article body; the two IRS-ICE articles are the same Sep-9 ruling (one is a thin partisan reframe repeating "the new report says" ~6×); ~9% of sampled articles carry AI tells, ~5% imply first-hand outreach ("did not respond to a request for comment"). The dominant risk is the **volume of AI rewrites** (classic "scaled content").
- **Code fixes (deployable):** `ArticleSanitizer` now strips placeholder tokens (`[[…]]`, `[LINK]`, `{{…}}`, …) in `clean()`/`cleanText()` + new `hasPlaceholder()`; `Rewriter` prompt hardened (never emit placeholders; never claim we contacted a source unless attributed). UX: homepage shop moved below the newsletter + trimmed 8→4; nav "Free Gifts" de-emphasized (no gold CTA); the auto email **modal no longer pops over articles** (`audience.js`) — the biggest mobile-reading interruption. Inline/end captures + push bar kept.
- **New retired-URL handling:** `post_redirects` table + `PostRedirect` model; `PostController` + the `/post/{slug}` `missing()` handler 301 retired slugs to their replacement.
- **New reversible commands (run on server):** `content:scrub-placeholders` (dry → `--apply` with JSON backup → `--restore`), `posts:merge {retired} {survivor}` (unpublish→archived + 301, `--undo`), `posts:review-flags` (read-only editorial list: outreach/AI-tell/thin/no-source/duplicate → CSV+JSON). Run order in `ADSENSE_AUDIT.md` §3.
- **Not done (needs user):** deploy + run the commands; re-edit/retire the ~200 AI-tell and ~110 outreach articles; reduce publishing volume; manual one-word fix of "ICE data sharing **with ICE**" in the survivor. **No claim of AdSense approval; site not submitted for review.**
