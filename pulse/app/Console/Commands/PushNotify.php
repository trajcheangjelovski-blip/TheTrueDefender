...
class PushNotify extends Command
{
    public function handle(): int
    {
        ...
        // Breaking news bypasses the rate limit — it's urgent, send it immediately.
        $breaking = (clone $fresh())->breakingActive()->latest('published_at')->first();

        if (! $breaking) {
            // Non-breaking: honour the interval.
